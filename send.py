...
def send(conf, receive_name, filename):
    ...
    if receive_name not in conf["publicKeys"]:
        ...
        if input() == "n":
            ...
            utils.add_public_key(conf, receive_name, pk)
    else:
    ...
    encrypted_shared_key = utils.get_encrypted_shared_key(key, pk)
    ...
    with open(filename, 'rb') as f:
        for chunk in iter(partial(f.read, utils.CHUNK_SIZE), b''):
